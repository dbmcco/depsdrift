...
from depsdrift.contracts import format_default_contract_block
from depsdrift.drift import compute_deps_drift
from depsdrift.git_tools import get_git_root, get_working_changes
from depsdrift.specs import DepsdriftSpec, extract_depsdrift_spec, parse_depsdrift_spec
from depsdrift.workgraph import Workgraph, find_workgraph_dir
...
def _maybe_write_log(wg: Workgraph, task_id: str, report: dict) -> None:
    ...
    if not findings:
        msg = "Depsdrift: OK (no findings)"
    else:
        kinds = ", ".join(sorted({str(f.get("kind")) for f in findings}))
        msg = f"Depsdrift: {score} ({kinds})"
        if recs:
...
def _maybe_create_followups(wg: Workgraph, report: dict) -> None:
    ...
    spec = DepsdriftSpec.from_raw(spec_raw)
    ...
    for f in findings:
        ...
        if kind != "lock_not_updated":
            continue

        follow_id = f"drift-deps-{task_id}"
        title = f"deps: {task_title}"
        touch = list(spec.manifests or []) + list(spec.locks or [])
        desc = (
            "Update dependency lockfile(s) to match manifest changes.\n\n"
            "Context:\n"
            f"- Origin task: {task_id}\n"
            f"- Finding: {f.get('summary')}\n\n"
            + format_default_contract_block(mode="core", objective=title, touch=touch)
            + "\n"
            + (report.get("_depsdrift_block") or "").strip()
            + "\n"
        )
        wg.ensure_task(
            task_id=follow_id,
...
def cmd_wg_check(args: argparse.Namespace) -> int:
    ...
    raw_block = extract_depsdrift_spec(description)
    if raw_block is None:
        report = {
            ...
            "score": "green",
            "spec": None,
            "telemetry": {"note": "no depsdrift block"},
            "findings": [],
            "recommendations": [],
        }
        if args.json:
    ...
    try:
        spec_raw = parse_depsdrift_spec(raw_block)
        spec = DepsdriftSpec.from_raw(spec_raw)
        ...
        report = {
            ...
            "findings": [
                {
                    "kind": "invalid_depsdrift_spec",
                    "severity": "warn",
                    "summary": "depsdrift block present but could not be parsed",
                }
            ],
            "recommendations": [
                {
                    "priority": "high",
                    "action": "Fix the depsdrift TOML block so it parses",
                    "rationale": "Depsdrift can only advise on lock drift when it can read the configuration.",
                }
            ],
        }
        report["_depsdrift_block"] = f"```depsdrift\n{raw_block}\n```"
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="depsdrift")
    ...
    check = wg_sub.add_parser("check", help="Check for dependency lock drift (requires a depsdrift block in the task)")
